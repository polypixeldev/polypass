...
class ListItemState with _$ListItemState {
  const factory ListItemState({
    required ListItemMode mode,
    required Key? masterKey
  }) = _ListItemState;

  factory ListItemState.empty() => const ListItemState(
...
class ListItemEvent with _$ListItemEvent {
  ...
  const factory ListItemEvent.masterKeyChanged(Key? masterKey) = MasterKeyAcquiredEvent;
}
...
class ListItemBloc extends Bloc<ListItemEvent, ListItemState> {
  ...
  void _onMasterKeyAcquired(MasterKeyAcquiredEvent event, Emitter<ListItemState> emit) {
    emit(state.copyWith(
