...
class EditFormEvent with _$EditFormEvent {
  ...
  const factory EditFormEvent.usernameChanged(String username) =
      UsernameChangedEvent;
  const factory EditFormEvent.passwordChanged(String password) =
...
class EditFormBloc extends Bloc<EditFormEvent, EditFormState> {
  ...
  void _onUsernameChanged(
      UsernameChangedEvent event, Emitter<EditFormState> emit) {
    emit(state.copyWith(username: event.username));
  }

  void _onPasswordChanged(
      PasswordChangedEvent event, Emitter<EditFormState> emit) {
  ...
  void _onFailedEvent(FailedEvent event, Emitter<EditFormState> emit) {
    emit(state.copyWith(submitted: false, editedItem: null));
  }
}
