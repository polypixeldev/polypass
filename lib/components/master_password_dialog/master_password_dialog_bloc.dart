...
class MasterPasswordDialogState with _$MasterPasswordDialogState {
  ...
  const factory MasterPasswordDialogState(
      ...
      required MasterPasswordDialogStatus status}) = _MasterPasswordDialogState;
...
class MasterPasswordDialogEvent with _$MasterPasswordDialogEvent {
  const factory MasterPasswordDialogEvent.masterPasswordChanged(
      String masterPassword) = MasterPasswordChangedEvent;
  const factory MasterPasswordDialogEvent.masterPasswordSubmitted() =
      MasterPasswordSubmittedEvent;
  const factory MasterPasswordDialogEvent.canceled() = CanceledEvent;
}

class MasterPasswordDialogBloc
    extends Bloc<MasterPasswordDialogEvent, MasterPasswordDialogState> {
  MasterPasswordDialogBloc({required this.vaultFile})
      : super(MasterPasswordDialogState.empty()) {
    on<MasterPasswordDialogEvent>((event, emit) {
      event.map(
          ...
          masterPasswordSubmitted: (event) =>
              _onMasterPasswordSubmitted(event, emit),
  ...
  void _onMasterPasswordChanged(MasterPasswordChangedEvent event,
      Emitter<MasterPasswordDialogState> emit) {
    emit(state.copyWith(masterPassword: event.masterPassword));
  }

  void _onMasterPasswordSubmitted(MasterPasswordSubmittedEvent event,
      Emitter<MasterPasswordDialogState> emit) {
    ...
    if (vaultFile.header.testMagic(derivedKey, vaultFile.contents.iv)) {
      emit(state.copyWith(
          ...
          masterKey: vaultFile.header
              .decryptMasterKey(derivedKey, vaultFile.contents.iv),
          derivedKey: derivedKey));
    } else {
      emit(state.copyWith(status: MasterPasswordDialogStatus.failed));
    }
  }

  void _onCanceled(
      CanceledEvent event, Emitter<MasterPasswordDialogState> emit) {
    emit(state.copyWith(status: MasterPasswordDialogStatus.canceled));
  }
}
